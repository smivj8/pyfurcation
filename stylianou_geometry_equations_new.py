...
parameters = [parent_radius, parent_length = 0, daughter_radius, daughter_length, 
...
def parent_pipe(phi_s, phi_c, parameters):
    """
...
def merging_pipe_positive_z(phi_s, phi_c, parameters):
    """
...
def merging_pipe_negative_z(phi_s, phi_c, parameters):
    """
...
def daughter_pipe_positive_z(phi_s, phi_c, parameters):
    """
    ...
    branching_angle = parameters[5]
    #Calculate xyz coordinates and pack into numpy array
    xyz_m_pos = merging_pipe_positive_z(branching_angle, phi_c, parameters) #get xyz_m_pos coords
...
def daughter_pipe_negative_z(phi_s, phi_c, parameters):
    """
    ...
    branching_angle = parameters[5]
    #Calculate xyz coordinates and pack into numpy array
    xyz_m_neg = merging_pipe_negative_z(branching_angle, phi_c, parameters) #get xyz_m_neg coords
    zeta = outer_radius * np.tan(phi_s - branching_angle) #cartesian transform of phi_s
...
def get_position_alpha(parameters):
    ...
    position_alpha = parent_pipe(0, carina_angle/2, parameters) #(A.16)
...
def get_position_gamma(parameters):
    ...
    position_gamma = daughter_pipe_positive_z(phi_gamma, -1*np.pi/2, parameters) #(A.17)
...
####################################################################################################
#                                                                                                  #
#            Functions for defining the positive and negative position vectors of the              #
#                    complete bifurcation unit from the individual dections                        #
#                                                                                                  #
####################################################################################################
...
def get_r_parent(phi_s, phi_c, parameters):
    ...
    if phi_s >= phi_s_min and phi_s <= 0:
        r_parent = parent_pipe(phi_s, phi_c, parameters)
    else:
...
def get_r_positive(phi_s, phi_c, parameters):
    ...
    if phi_s >= 0 and phi_s <= branching_angle:
        r_positive = merging_pipe_positive_z(phi_s, phi_c, parameters)
    elif phi_s > branching_angle and phi_s <= phi_s_max:
        r_positive = daughter_pipe_positive_z(phi_s, phi_c, parameters)
    else:
        print("Error: Could not evaluate r_positive given phi_s. Printing Error log... \n")
        print(f"phi_s value: {phi_s}")
        r_positive = None
...
def get_r_negative(phi_s, phi_c, parameters):
    ...
    #construct the piecewise equation (A.15), ensurung phi_s is in range
    if phi_s >= 0 and phi_s <= branching_angle:
        r_negative = merging_pipe_positive_z(phi_s, phi_c, parameters)
    elif phi_s > branching_angle and phi_s <= phi_s_max:
        r_negative = daughter_pipe_positive_z(phi_s, phi_c, parameters)
    else:
        print("Error: Could not evaluate r_positive given phi_s. Printing Error log... \n")
        print(f"phi_s value: {phi_s}")
        r_negative = None
...
def parent_pipe_normal(phi_s, phi_c, parameters):
    """
    ...
    """
    #Get the xyz value
    xyz_p = parent_pipe(phi_s, phi_c, parameters)
...
def merging_pipe_positive_z_normal(phi_s, phi_c, parameters):
    """
    Construct the xyz position of the merging pipe section in the positive-z 
    range at parameteric location (phi_s, phi_c) for given parameters.
    ...
    xyz_m_pos = merging_pipe_positive_z(phi_s, phi_c, parameters)
...
def merging_pipe_negative_z_normal(phi_s, phi_c, parameters):
    """
    Construct the xyz position of the merging pipe section in the negative-z 
    range at parameteric location (phi_s, phi_c) for given parameters. 
    Negative-z variation of Equations (A.4), (A.5), (A.6), pg. 699
    ...
    outer_radius = parameters[4]
    #Calculate xyz coordinates and pack into numpy array
    xyz_m_neg = merging_pipe_negative_z(phi_s, phi_c, parameters)
    #Calculate unit vectors normal to the pointcloud
    val = transition_radius(phi_s, parameters) * np.sin(phi_c)
    scalar = 1.1
...
def daughter_pipe_positive_z_normal(phi_s, phi_c, parameters):
    """
    Construct the xyz position of the daughter pipe section in the positive-z 
    range at parameteric location (phi_s, phi_c) for given parameters. 
    Positive-z variation of Equations (A.12), (A.13), (A.14), pg. 699
    ...
    branching_angle = parameters[5]
    #Calculate xyz coordinates
    xyz_d_pos = daughter_pipe_positive_z(phi_s, phi_c, parameters)
    #Calculate unit vectors normal to the pointcloud
    scalar = 1.1
    zeta = outer_radius * np.tan(phi_s - branching_angle) #cartesian transform of phi_s
    ...
    xyz_m_dummy_pos, _ = merging_pipe_positive_z(branching_angle, phi_c, dummy_parameters)
...
def daughter_pipe_negative_z_normal(phi_s, phi_c, parameters):
    """
    Construct the xyz position of the daughter pipe section in the negative-z 
    range at parameteric location (phi_s, phi_c) for given parameters. 
    Negative-z variation of Equations (A.12), (A.13), (A.14), pg. 699
    ...
    branching_angle = parameters[5]
    #Calculate xyz coordinates
    xyz_d_neg = daughter_pipe_negative_z(phi_s, phi_c, parameters)
    #Calculate unit vectors normal to the pointcloud
    zeta = outer_radius * np.tan(phi_s - branching_angle) #cartesian transform of phi_s
    scalar = 1.1
    dummy_parameters = parameters
    dummy_parameters[0] = parameters[0] * scalar
    dummy_parameters[2] = parameters[2] * scalar
    xyz_m_dummy_neg, _ = merging_pipe_negative_z(branching_angle, phi_c, dummy_parameters)
